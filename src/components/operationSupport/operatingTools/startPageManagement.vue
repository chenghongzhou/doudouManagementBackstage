...
export default {
    ...
    methods: {
        ...
        uploading(event, type) {
            ...
            if(type == 0){
                ...
                _this.formData.file_pic = event.target.files[0];
            }else if(type ==1){
                var windowURL = window.URL || window.webkitURL; 
                _this.editData.pic = windowURL.createObjectURL(
                    event.target.files[0]
                );
                _this.editData.file_pic = event.target.files[0];
            }
        },
        // 添加启动页
        addPageSure(type) {
            ...
            if (type == 0) {
                ...
                axios.post(allget+'/GlobalSet/addStartPage', formData, config)
                    .then((res) => {
                        if(res.data.ret){
                            baseConfig.successTipMsg(_this, res.data.msg);
        ...
        sureEdit(type) {
            ...
            if(type == 0){
                ...
                axios.post(allget+'/GlobalSet/editStartPage', formData, config)
                    .then((res) =>{
                        if(res.data.ret){
                            baseConfig.successTipMsg(_this, res.data.msg);
    ...
    mounted() {
        ...
        this.$nextTick(function() {
            ...
            _this.getTableData();
            _this.operation_name = store.state.user.name; 
        });
    }
};
</script>
