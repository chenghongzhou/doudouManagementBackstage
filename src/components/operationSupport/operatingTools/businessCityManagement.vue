...
export default {
	...
	methods: {
		...
		*/ 
		...
		getOneData() {
			var _this = this ;
			var url = allget+'/NewProp/getProductList';
			var params = {
		...
		deleteChange(obj) {
			var _this = this;
			var url = allget+'/NewProp/dealWithProduct';
			var params = {
				type: 0,
				id: obj.id,
			};
    ...
      	}, 
		editorSure(type) {
			...
			if(type==0) {
				...
				var url = allget+'/NewProp/editProductList';				
		...
		addSure(type) {
			...
			if(type==0) {
				...
				var url = allget+'/NewProp/addProductList';
				let formData = new FormData();
				let config = { headers: { 'Content-Type': 'multipart/form-data' } };	
		...
		*/ 
		...
		getOneId() {
			var _this = this;
			var url = allget+'/NewProp/getPropList';
			var params = {};
			axios.get(url, { params: params })
				.then((res) => {
					var arr = [];
		...
		getTwoId() {
			var _this = this;
			var url = allget+'/NewProp/getPropAttireList';
			var params = {};
			axios.get(url, { params: params })
				.then((res) => {
					if(res.data.ret) {
		...
		getThreeId() {
			var _this = this;
			var url = allget+'/NewProp/getPropCarList';
			var params = {};
			axios.get(url, { params: params })
				.then((res) => {
					var arr = [];
