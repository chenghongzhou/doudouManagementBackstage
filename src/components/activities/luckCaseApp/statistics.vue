	...
	@tab-click="handleClick">
		...
		:style="{height:tabHeight+'px'}">
			...
			:height="tabSearchPageHeightOthers">
               ...
               <el-table-column label="宝箱类别" width="100">
                    <template slot-scope="scope">
                        <div slot="reference" class="name-wrapper">
                            <p v-if="scope.row.type==1">黄金宝箱</p>
                            <p v-else-if="scope.row.type==2">铂金宝箱</p>
                            <p v-else-if="scope.row.type==3">钻石宝箱</p>									
                        </div>
                    </template>
                </el-table-column>
				<el-table-column prop="uid" label="UID"></el-table-column>
                <el-table-column prop="to_uid" label="收礼者ID"></el-table-column>
                <el-table-column prop="gift_name" label="赠送礼物"></el-table-column>
                <el-table-column prop="gift_total" label="价值"></el-table-column>
                <el-table-column label="是否爆款" width="100">
                    <template slot-scope="scope">
                        <div slot="reference" class="name-wrapper">
                            <p v-if="scope.row.is_hot==1">是</p>
                            <p v-else-if="scope.row.is_hot==0">否</p>								
                        </div>
                    </template>
                </el-table-column>
   			</el-table> 
		...
		:style="{height:tabHeight+'px'}">
			...
			style="padding-bottom:0px;">
				...
				:model="formThree">
                    <!-- <el-form-item>
                        <span>宝箱等级：</span>
                        <el-select style="width:200px;" v-model="formThree.box">
                            <el-option label="黄金宝箱" value="1"></el-option>
                            <el-option label="铂金宝箱" value="2"></el-option>
                            <el-option label="钻石宝箱" value="3"></el-option>
                        </el-select>
			...
			:height="tabSearchPageHeightOthers">
               <el-table-column label="宝箱类别" width="100">
				...
				<el-table-column prop="join_num" label="参与人数"></el-table-column>
                <el-table-column prop="count" label="次数"></el-table-column>
                <el-table-column prop="chat_gold" label="收入(豆币)"></el-table-column>
                <el-table-column prop="chat_ticket" label="支出(豆币)"></el-table-column>
   			</el-table>  						
		    <el-col :span="24" class="toolbar">
				<el-pagination 
					layout="total,prev,pager,next,jumper" 
	...
	export default{
		...
		methods:{
			...
			searchConditionGift(){
                ...
                obj.box_type=_this.formOne.box;
            ...
            searchConditionComData(){
				var _this = this;
				var obj = {};
				obj.page=_this.formThree.page;
                obj.box_type=_this.formThree.box;
			...
			getOneData(){
				...
				axios.get(allget+'/NewLuckBox/getDateStat', {params: params})
				.then((res) => {
					if(res.data.ret == 1) {
						var oneTitleStatic = [];
						var arr = [];
						...
						oneTitleStatic = [
							...
							{identification:'count',name:'送出次数'},
							{identification:'hot_gift_name',name:'爆款'},
                            {identification:'chat_gold',name:'收入'},
                            {identification:'chat_ticket',name:'支出'},
						]
			...
			getTwoData(){
				...
				axios.get(allget+'/NewLuckBox/getUserStat', {params: params})
				.then((res) => {
					if(res.data.ret == 1) {
						_this.tabTwoData = res.data.data.stat_list;
					} else {
						baseConfig.warningTipMsg(_this, res.data.msg);
					}
				})
			...
			getThreeData(){
                var _this = this;
                var params=_this.searchConditionComData();
				axios.get(allget+'/NewLuckBox/getDateStat', {params: params})
				.then((res) => {
					...
					if(res.data.ret == 1) {
						...
						arr = [
                            {
                                type:'总计',
                                join_num:res.data.data.box_stat_total.join_num,
                                count:res.data.data.box_stat_total.count,
                                chat_gold:res.data.data.box_stat_total.chat_gold,
                                chat_ticket:res.data.data.box_stat_total.chat_ticket,
                            }
                        ];
                        var newArr = arr.concat(res.data.data.box_stat_list);
                        _this.tabThreeData = newArr;
                        console.log(_this.tabThreeData,106)
					} else {
						baseConfig.warningTipMsg(_this, res.data.msg);
					}
				})
