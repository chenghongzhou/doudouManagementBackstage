...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            let url = "/NewGoldenEgg/getPrizeList";
            let params = {
                page: _this.page,
            };
		...
		checkType(){
			var _this = this;
			_this.wp = [];
			let url = "/NewGoldenEgg/getGiftOrPropList";
            let params = {
		...
		addBannerSure(num){
			...
			if(num == 0){
				...
				axios.post(allget+'/NewGoldenEgg/setPrize', formData, config)
					.then((res) => {
						_this.listLoading = false;							
						if(res.data.ret == 1) {	
							_this.addNewloading.dialogFormVisible = true;
		...
		checkSort(box){
			var _this = this;
			_this.getSort = ['0'];
			let url = "/NewGoldenEgg/getPrizeSortList";
            let params = {
        ...
        deleteContentRecord(index, row){
            ...
            this.$confirm('此操作将永久删除, 是否继续?', '提示', {
                ...
                var url = '/NewGoldenEgg/deletePrize';
                let params = {
