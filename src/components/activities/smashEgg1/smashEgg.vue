    ...
    methods: {
		...
		addBannerSure(num){
			...
			if(num == 0){
				...
				axios.post(allget+'/NewEgg/setPrize', formData, config)
					.then((res) => {
						_this.listLoading = false;							
						if(res.data.ret == 1) {	
							_this.addNewloading.dialogFormVisible = true;
		...
		checklevel(box){
			var _this = this;
			_this.getSort = ['0'];
			let url = "/NewEgg/getPrizeLevelList";
            let params = {
		...
		checkSort(box){
			var _this = this;
			_this.getSort = ['0'];
			let url = "/NewEgg/getPrizeSortList";
            let params = {
