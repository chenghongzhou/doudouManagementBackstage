		...
		<el-dialog title="作弊后台添加" :visible.sync="formThree.dialogFormVisibleOther">
			<el-form :model="formThree">
				<el-form-item label="宝箱" :label-width="formLabelWidth">
					<el-select v-model="formThree.box" @change="checkSort">
						<el-option label="黄金宝箱" value="1"></el-option>
						<el-option label="白金宝箱" value="2"></el-option>
						<el-option label="氪金宝箱" value="3"></el-option>
					</el-select>
...
<script>
	...
	export default{
		...
		methods:{
			...
			getThreeData(){
       			var _this = this;
				axios.get(allget+'/NewTreasureBox/getCheatList', {params: {status:0,page:_this.formThree.page}})
				.then((res) => {
					if(res.data.ret == 1) {
			...
			getFourData(){
       			var _this = this;
				axios.get(allget+'/NewTreasureBox/getCheatList', {params: {status:1,page:_this.formFour.page}})
				.then((res) => {
					if(res.data.ret == 1) {
			...
			checkSort(){
				var _this = this;
				let url = "/NewTreasureBox/getAvailableLotteryList";
				let params = {
					box: _this.formThree.box
				};
			...
			addNoReal(num){
				...
				if(num == 0){
					...
					let url = "/NewTreasureBox/addCheat";
					let params = {
			...
			deleteNoReal(index, row){
				...
				this.$confirm('此操作将永久删除, 是否继续?', '提示', {
					...
					var url = '/NewTreasureBox/deleteCheat';
					let params = {
