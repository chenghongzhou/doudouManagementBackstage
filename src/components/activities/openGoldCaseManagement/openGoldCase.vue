    ...
    <section>
		...
		<el-dialog title="添加" :visible.sync="addNewloading.dialogFormVisible">
			<el-form :model="addNewloading.params">
				<el-form-item label="宝箱等级" :label-width="formLabelWidth">
					<el-select v-model="addNewloading.params.box" @change="checkSort">
						<el-option label="黄金宝箱" value="1"></el-option>
						<el-option label="白金宝箱" value="2"></el-option>
						<el-option label="氪金宝箱" value="3"></el-option>
					</el-select>
				...
				<el-form-item label="物品类型" :label-width="formLabelWidth">
					<el-select v-model="addNewloading.params.type" @change="checkType">
						...
						<el-option label="幸运星" value="4"></el-option>
					</el-select>
				</el-form-item> 
				...
				<el-form-item label="星星数量" :label-width="formLabelWidth" v-show="starBox == true">
					<el-input 
					style="width:250px"
					v-model="addNewloading.params.num" 
					auto-complete="off"  @input="sumStar()"></el-input>
				</el-form-item>
        ...
        <el-dialog title="编辑" :visible.sync="editorloading.dialogFormVisible">
			<el-form :model="editorloading.params">
				<el-form-item label="宝箱等级" :label-width="formLabelWidth">
					<el-select v-model="editorloading.params.box" disabled>
						<el-option label="黄金宝箱" value="1"></el-option>
						<el-option label="白金宝箱" value="2"></el-option>
						<el-option label="氪金宝箱" value="3"></el-option>
					</el-select>
				...
				<el-form-item label="星星数量" :label-width="formLabelWidth" disabled v-show="starBox == true">
					<el-input 
					style="width:250px"
				...
				<el-form-item label="排序ID" :label-width="formLabelWidth">
					<el-select v-model="editorloading.params.sort">
						<el-option 
							...
							:value="item"
							:key="index"
							v-for="(item,index) in getEditSort">
							</el-option>
					</el-select>
				</el-form-item> 
			</el-form>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            let url = "/NewTreasureBox/getLotteryList";
            let params = {
                box: _this.box,
                page: _this.page,
            };
		...
		checkType(){
			var _this = this;
			_this.wp = [];
			if(_this.addNewloading.params.type == 4){
				_this.wp = [{id: "0", name: "幸运星"}];
				_this.starBox = true;
				return false;
			}else{
				_this.starBox = false;
			};
			let url = "/NewTreasureBox/getGiftOrPropList";
            let params = {
		...
		checkSort(){
			var _this = this;
			_this.getSort = [];
			let url = "/NewTreasureBox/getLotterySortList";
            let params = {
		...
		sumStar(){
			var _this = this;
			var chatGold = _this.addNewloading.params.num;
			var reg = /^(0|[1-9][0-9]*)$/;
			if(reg.test(chatGold)){
				_this.addNewloading.params.chat_gold = chatGold*10;
			}else{
				baseConfig.warningTipMsg(_this, '请输入正确的数字');
			};
		},
		//上传图片
		uploading(event){
		...
		addBannerSure(num){
			...
			if(num == 0){
				...
				formData.append('box', _this.addNewloading.params.box);
				...
				axios.post(allget+'/NewTreasureBox/setLottery', formData, config)
					.then((res) => {
						_this.listLoading = false;							
						if(res.data.ret == 1) {	
							_this.addNewloading.dialogFormVisible = true;
		...
		modify(index,rows){
			...
			this.checkEditSort(rows.box);
			this.editorloading.params.box = rows.box;
		...
		checkEditSort(box){
            ...
            axios.get(allget+url, {params: params})
                .then((res) => {
                    if (res.data.ret) {
                        _this.getEditSort = res.data.data;
                    } else {
                        baseConfig.warningTipMsg(_this, res.data.msg);
                    }
                })
		...
		editorBannerSure(num){
			...
			if(num == 0){
				...
				formData.append('box', _this.editorloading.params.box);
        ...
        deleteContentRecord(index, row){
            ...
            this.$confirm('此操作将永久删除, 是否继续?', '提示', {
                ...
                var url = '/NewTreasureBox/deleteLottery';
                let params = {
