    ...
    <section>
		...
		<el-dialog title="添加" :visible.sync="addNewloading.dialogFormVisible">
			<el-form :model="addNewloading.params"> 
                ...
                <el-form-item label="宝箱类型" :label-width="formLabelWidth">
					<el-select v-model="addNewloading.params.box">
						<el-option label="黄金宝箱" value="1"></el-option>
						<el-option label="钻石宝箱" value="2"></el-option>
					</el-select>
				</el-form-item> 
    ...
    methods: {
		...
		addBannerSure(num){
			...
			if(num == 0){
				...
				formData.append('gift_id', _this.addNewloading.params.prop_id);
				...
				axios.post(allget+'/NewBox/setPrize', formData, config)
					.then((res) => {
						_this.listLoading = false;							
						if(res.data.ret == 1) {	
							_this.addNewloading.dialogFormVisible = true;
		...
		modify(index,rows){
			...
			this.editorloading.params.type = '1';
		...
		editorBannerSure(num){
			...
			if(num == 0){
				...
				formData.append('gift_id', _this.editorloading.params.prop_id);
