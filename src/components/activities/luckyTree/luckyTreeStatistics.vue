	...
	export default{
		...
		methods:{
			...
			getOneData(){
				...
				axios.get(allget+'/NewMoneyTree/getDateStat', {params: params})
				.then((res) => {
					if(res.data.ret == 1) {
						var oneTitleStatic = [];
						var arr = [];
			...
			getTwoData(){
				...
				axios.get(allget+'/NewMoneyTree/getUserStat', {params: params})
				.then((res) => {
					if(res.data.ret == 1) {
			...
			getThreeData(){
       			var _this = this;
				axios.get(allget+'/NewMoneyTree/getCheatList', {params: {status:0,page:_this.formThree.page}})
				.then((res) => {
					if(res.data.ret == 1) {
			...
			getFourData(){
       			var _this = this;
				axios.get(allget+'/NewMoneyTree/getCheatList', {params: {status:1,page:_this.formFour.page}})
				.then((res) => {
					if(res.data.ret == 1) {
			...
			checkSort(){
				var _this = this;
				let url = "/NewMoneyTree/getAvailablePrizeList";
				let params = {
			...
			deleteNoReal(index, row){
				...
				this.$confirm('此操作将永久删除, 是否继续?', '提示', {
					...
					var url = '/NewMoneyTree/deleteCheat';
					let params = {
