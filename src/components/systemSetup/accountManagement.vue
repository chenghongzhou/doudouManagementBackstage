...
export default {
    ...
    methods:{
        ...
        editDialogShow(index, rows) {
            ...
            _this.editDialog.phone = rows.phone;
        ...
        editDialogBtn(type) {
            ...
              	formData.append('phone', _this.editDialog.phone);
        ...
        addDialogBtn(type) {
            ...
              	formData.append('phone', _this.addDialog.phone);
        ...
                };
                if(formData.get('user_name')!='' && formData.get('real_name')!='' && formData.get('pass_word')!='' && formData.get('auth_code')!='' && formData.get('page_auth_code')!='') {
                    axios.post(allget+url, formData, config)
                        .then((res) => {
                            ...
                            if(res.data.ret) {
                                ...
                                // 新添成功进行input弹窗内容清空
                                _this.addDialog.user_name = '';
                                _this.addDialog.real_name = '';
                                _this.addDialog.phone = '';
                                _this.addDialog.auth_code = '';
                                _this.addDialog.page_auth_code = '';
                            } else {
                                baseConfig.warningTipMsg(_this, res.data.msg);
                            }
                        })
                    ...
                    baseConfig.warningTipMsg(_this, '请认真填写啦~');
                }
            }
        },
        // 渠道选择编辑的弹窗
